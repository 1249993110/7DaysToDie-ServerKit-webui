...
const getData = async () => {
    const data = await api.getSettings('TeleportHome');
...
const reset = async () => {
    try {
        if (await myconfirm('确定重置配置吗?')) {
            await api.resetSettings('TeleportHome', formModel);
            await getData();
...
const variables = ['HomeName', 'TeleInterval', 'CooldownSeconds', 'EntityId', 'PlatformId', 'PlayerName'];
